...
void Tsuro::putPlayersToGrid() {
    ...
    for (int i = 0; i < numberOfPlayers; ++i) { // puts players to grid frame by random position
        ...
        tsuroGrid.setGridInCell(tsuroPlayers[i].getPlayerPositionY(), tsuroPlayers[i].getPlayerPositionX(),
                                tsuroPlayers[i].getPlayerName());
...
void Tsuro::checkCardEnvironment(int cardX, int cardY, int playerIndex) {
    ...
    for (int i = 0; i < 8; ++i) { // checks card environments for there is player
        for (int j = 0; j < 18; ++j) {
            ...
            if (tsuroGrid.getGridInCell(boardY - 1 + i, boardX - 3 + j) == tsuroPlayers[playerIndex].getPlayerName() &&
                (((i == 0 || i == 8 - 1) || (j < 3)) || (j > 14))) {
                posX = boardX - 3 + j;
                posY = boardY - 1 + i;
                // if player is on the frame , puts players position 'o'
                if (posX == 0 || posY == 0 || posY == tsuroGrid.getGridHeight() - 1 ||
                    posX == tsuroGrid.getGridWidth() - 1) {
                    tsuroGrid.setGridInCell(posY, posX, 'o');
                } else { // is not on the frame put previous port number to  players positions
                    tsuroGrid.setGridInCell(posY, posX,
                                            static_cast<char>(tsuroPlayers[playerIndex].getPlayerBackPort() +
                                                              charToInt));
                }
            }
        }
    ...
    for (int k = 0; k < cardHeight; ++k) { // finding card port for players port
        for (int i = 0; i < cardWidth; ++i) {
            if (tsuroGrid.getGridInCell(boardY + k, boardX + i) == path + charToInt &&
                ((k == 0 || k == 5) || (i == 0 || i == 11))) {
                tsuroGrid.setGridInCell(boardY + k, boardX + i, tsuroPlayers[playerIndex].getPlayerName());
                // if player port is near the frame
                if (boardY + k == 1 || boardY + k == tsuroGrid.getGridHeight() - 2 || boardX + i == 2 ||
                    boardX + i == tsuroGrid.getGridWidth() - 3) {
                    displayCurrentBoard();
                    cout << "Player " << tsuroPlayers[playerIndex].getPlayerName() << " lost!" << endl;
                    tsuroGrid.setGridInCell(boardY + k, boardX + i,
                                            static_cast<char>(tsuroPlayers[playerIndex].getPlayerBackPort() +
                                                              charToInt));
